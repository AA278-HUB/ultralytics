...
from ultralytics.utils import LOGGER, TryExcept, plt_settings, SimpleClass
...
def My_bbox_mpdiou(box1, box2, xywh=True, img_size=640, eps=1e-7):
    """
    计算 MPDIoU
    img_size: 图像的分辨率（假设是正方形，或者传入 w^2 + h^2 的结果）
    """
    # 1. 坐标转换
    if xywh:  # (x_center, y_center, w, h) -> (x1, y1, x2, y2)
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
    ...
    # 3. 计算 MPDIoU 特有的距离项
    # s2 是图像或特征图的 diagonal squared (W^2 + H^2)
    # 如果 box 坐标已经是归一化的 (0-1)，s2 可以直接设为 1^2 + 1^2 = 2
    s2 = img_size ** 2 if isinstance(img_size, int) else img_size

    d1 = (b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2
    d2 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2

    return iou - d1 / s2 - d2 / s2
