...
def bbox_iou(
    ...
    # Get the coordinates of bounding boxes
    if ShapeIoU :
       Iou= shape_iou(box1,box2,xywh=True, scale=0.5, eps=1e-7)
       return Iou
...
class ConfusionMatrix(DataExportMixin):
    ...
    def plot(self, normalize: bool = True, save_dir: str = "", on_plot=None):
        ...
        # ============================================================
        # 关键修复点：
        # 1. normalize 必须在完整 (nc+1 x nc+1) 矩阵上进行
        # 2. background 只在“显示阶段”裁掉
        # ============================================================
        full = self.matrix
        array = full / ((full.sum(0).reshape(1, -1) + 1e-9) if normalize else 1)

        if self.task != "classify":
            array = array[:-1, :-1]  # 仅用于显示，去掉 background
