...
def bbox_alpha_iou(
    ...
    """
    计算 Alpha-IoU 及其变体，兼容 Ultralytics 广播机制。
    """
    # 1. 坐标转换与切片
    if xywh:
    ...
    # 3. Alpha-IoU 核心逻辑
    iou = inter / union
    # 使用 power 进行 Alpha 处理，并添加 epsilon 防止数值不稳定
    iou = torch.pow(iou + eps, alpha)
    ...
    if GIoU or DIoU or CIoU:
        ...
        if CIoU or DIoU:
            c2 = (cw.pow(2) + ch.pow(2)) ** alpha + eps
            rho_x = (b2_x1 + b2_x2 - b1_x1 - b1_x2).abs()
            rho_y = (b2_y1 + b2_y2 - b1_y1 - b1_y2).abs()
            rho2 = ((rho_x.pow(2) + rho_y.pow(2)) / 4) ** alpha
            ...
            c_area = (cw * ch + eps).maximum(union)
